...
const App = () => {
  ...
  // Função para adicionar uma cor à paleta atual
  const handleAddColor = (e) => {
    const newColor = e.target.value;
    if (currentColors.length < 5 && !currentColors.includes(newColor)) {
  ...
  const handleRemoveColor = (colorToRemove) => {
    setCurrentColors(currentColors.filter(color => color !== colorToRemove));
  };
  ...
  const handleImageUpload = (e) => {
    ...
    if (file && currentImages.length < 4) {
      ...
      reader.onloadend = () => {
        setCurrentImages([...currentImages, reader.result]);
      };
  ...
  const savePalette = () => {
    if (currentColors.length > 0 || currentImages.length > 0 || currentNotes.trim() !== '') {
      const newPalette = {
        id: Date.now(),
  ...
  const loadPalette = (palette) => {
    ...
    setCurrentImages(palette.images);
        ...
        {!isHistoryView ? (
          ...
          <div className="w-full max-w-4xl bg-white p-8 rounded-xl shadow-lg">
            ...
            <section className="mb-8">
              <h3 className="text-xl font-semibold text-gray-700 mb-2">Paleta de Cores ({currentColors.length}/5)</h3>
              <div className="flex items-center gap-4 flex-wrap">
                ...
                {currentColors.map((color, index) => (
                  <div
                    ...
                    className="w-16 h-16 rounded-full shadow-md cursor-pointer transition-transform transform hover:scale-105 relative"
                    style={{ backgroundColor: color }}
                    title="Clique para remover"
                  >
                    <span className="absolute inset-0 flex items-center justify-center text-white text-sm font-bold opacity-0 hover:opacity-100 transition-opacity">
                      {color.toUpperCase()}
                    </span>
                  </div>
                ))}
            ...
            {/* Seção de Fotos */}
            <section className="mb-8">
              <h3 className="text-xl font-semibold text-gray-700 mb-2">Fotos de Referência ({currentImages.length}/4)</h3>
